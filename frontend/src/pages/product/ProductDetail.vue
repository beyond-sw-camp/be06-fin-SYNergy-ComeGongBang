<template>
  <div
    class="appContainer"
    style="
      --mobile-the-snackbar-bottom: 60px;
      --desktop-header-menu-top: 27px;
      --desktop-header-sticky-height: 0px;
    "
    data-v-261d543d=""
  >
    <div id="MoveToTopTrigger" data-v-261d543d=""></div>
    <div class="appContents" data-v-261d543d="">
      <!--[-->
      <div>
        <div class="ProductDetail" data-v-6da505b4="">
          <div class="flex justify-center" data-v-6da505b4="">
            <div><CarouselProductDetailComponent/></div>
            <div><ProductDetailPaymentComponent/></div>
          </div>
          <div class="ProductDetail__tab" data-v-6da505b4="">
            <div
              class="BaseTabs BaseTabs__size--large BaseTabs__style--outline BaseTabs--width BaseTabs__border--top BaseTabs__border--bottom BaseTabs__align--left"
              style="--BaseTabs-item-length: 4; --BaseTabs-margin-x: 0"
              data-v-de0565e1=""
              data-v-6da505b4=""
            >
            <!-- 페이지네이션 탭 -->
              <div class="BaseTabs__inner" role="tablist" data-v-de0565e1="">
                <!-- 작품 정보 탭 -->
                <div
                  class="BaseTab BaseTab--active"
                  style="--BaseTab-width: 170;"
                  role="tab"
                  border-top="true"
                  border-bottom="true"
                  data-v-47c662bd=""
                  data-v-de0565e1=""
                >
                  <div class="BaseTab__contents" data-v-47c662bd="">
                    <div class="BaseTab__prefix" data-v-47c662bd="">
                    </div>
                    <div
                      class="BaseFontVariable"
                      data-v-9dbc8be1=""
                      data-v-47c662bd=""
                    >
                      <div class="BaseFontVariable__text" data-v-9dbc8be1="">
                        <span
                          data-v-9dbc8be1=""
                          >작품정보</span
                        >
                      </div>
                    </div>
                  </div>
                </div>
                <!-- 후기 탭 -->
                <div
                  class="BaseTab"
                  style="--BaseTab-width: 170"
                  role="tab"
                  border-top="true"
                  border-bottom="true"
                  data-v-47c662bd=""
                  data-v-de0565e1=""
                >
                  <div class="BaseTab__contents" data-v-47c662bd="">
                    <div
                      class="BaseFontVariable"
                      data-v-9dbc8be1=""
                      data-v-47c662bd=""
                    >
                      <div class="BaseFontVariable__text" data-v-9dbc8be1="">
                        <span
                          data-v-9dbc8be1=""
                          >후기</span
                        >
                      </div>
                      <span
                        class="flex-auto inline-flex items-center"
                        data-v-9dbc8be1=""
                        ><!--[--><span class="BaseTab__label" data-v-47c662bd=""
                          >238</span
                        ><!--]--></span
                      >
                    </div>
                  </div>
                </div>
                <!-- 문의 탭 -->
                <div
                  class="BaseTab"
                  style="--BaseTab-width: 170"
                  role="tab"
                  border-top="true"
                  border-bottom="true"
                  data-v-47c662bd=""
                  data-v-de0565e1=""
                >
                  <div class="BaseTab__contents" data-v-47c662bd="">
                    <div
                      class="BaseFontVariable"
                      data-v-9dbc8be1=""
                      data-v-47c662bd=""
                    >
                      <div class="BaseFontVariable__text" data-v-9dbc8be1="">
                        <span
                          data-v-9dbc8be1=""
                          >문의</span
                        >
                      </div>
                    </div>
                  </div>
                </div>
                <!-- 작가님 정보 탭 -->
                <div
                  class="BaseTab"
                  style="--BaseTab-width: 170"
                  role="tab"
                  border-top="true"
                  border-bottom="true"
                  data-v-47c662bd=""
                  data-v-de0565e1=""
                >
                  <div class="BaseTab__contents" data-v-47c662bd="">
                    <div
                      class="BaseFontVariable"
                      data-v-9dbc8be1=""
                      data-v-47c662bd=""
                    >
                      <div class="BaseFontVariable__text" data-v-9dbc8be1="">
                        <span
                          data-v-9dbc8be1=""
                          >작가님 정보</span
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div class="pb-[40px]" data-v-6da505b4="">
            <div
              class="w-[680px] mt-[16px]"
              data-v-6da505b4=""
              style="width: 100%"
            >
              <div id="DESC" data-v-6da505b4="">
                <div
                  class="ProductDetailDescription"
                  style="--description-height: 2000px"
                  data-v-6da505b4=""
                  data-v-1d911075=""
                >

                  <!-- 여긴가 -->
                  <div
                    style=""
                    class="ProductDetailDescription__more"
                    data-v-1d911075=""
                  >
                    <button
                      type="outline"
                      class="CoreButton CoreButton--block BaseButtonRectangle subtitle3-bold-small BaseButtonRectangle__outline"
                      style="
                        background-color: #ffffff;
                        color: #505fcd;
                        height: 44px;
                        flex-direction: row-reverse;
                        --core-button-padding-x: 16;
                        font-weight: bold;
                        --button-rectangle-border-color: #505fcd;
                      "
                      data-v-524f63ea=""
                      data-v-7940d6dd=""
                      data-v-1d911075=""
                    >
                      <!----><!--[-->
                    <svg
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg"
                        style="
                          width: 20px;
                          height: 20px;
                          opacity: 1;
                          fill: currentColor;
                          --BaseIcon-color: #333333;
                          margin-left: 2px;
                        "
                        class="BaseIcon CoreButton__icon"
                        data-v-6d2bd019=""
                        data-v-524f63ea=""
                      >
                        <g clip-path="url(#clip0_124_2949)">
                          <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M17.4697 8.46973L18.5304 9.53039L12.5304 15.5304C12.2641 15.7967 11.8475 15.8209 11.5538 15.603L11.4697 15.5304L5.46973 9.53039L6.53039 8.46973L12.0001 13.9391L17.4697 8.46973Z"
                          ></path>
                        </g>
                        <defs>
                          <clipPath id="clip0_124_2949">
                            <rect width="24" height="24"></rect>
                          </clipPath>
                        </defs>
                      </svg>
                      <div class="inline-flex items-center" data-v-524f63ea="">
                        <!--[--><!--]--><span
                          class="CoreButton__text"
                          data-v-524f63ea=""
                          >작품 정보 더보기</span
                        >
                      </div>
                      <!--]-->
                    </button>
                  </div>
                </div>
              </div>
              <div class="mt-[36px]" data-v-6da505b4="">
                <div data-v-6da505b4="">
                  <div
                    class="BaseSubHeader BaseSubHeader--clickable"
                    style="
                      --sub-header-padding-left: 0;
                      --sub-header-padding-right: 0;
                      --sub-header-min-height: 56;
                    "
                    data-v-b42a95d0=""
                  >
                    <!---->
                    <div class="BaseSubHeader__contents" data-v-b42a95d0="">
                      <!--[--><span class="subtitle3-bold-small"
                        >작품 정보제공 고시</span
                      ><!--]-->
                    </div>
                    <div class="BaseSubHeader__append" data-v-b42a95d0="">
                      <!--[--><svg
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg"
                        style="
                          width: 20px;
                          height: 20px;
                          opacity: 1;
                          fill: currentColor;
                          --BaseIcon-color: #666666;
                        "
                        class="BaseIcon"
                        data-v-6d2bd019=""
                      >
                        <g clip-path="url(#clip0_124_2949)">
                          <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M17.4697 8.46973L18.5304 9.53039L12.5304 15.5304C12.2641 15.7967 11.8475 15.8209 11.5538 15.603L11.4697 15.5304L5.46973 9.53039L6.53039 8.46973L12.0001 13.9391L17.4697 8.46973Z"
                          ></path>
                        </g>
                        <defs>
                          <clipPath id="clip0_124_2949">
                            <rect width="24" height="24"></rect>
                          </clipPath>
                        </defs></svg
                      ><!--]-->
                    </div>
                  </div>
                  <!-- 작품 제공정보 토글 -->
                  <div
                    style="display: none"
                    class="body1-regular-small pb-[16px]"
                  >
                    <!--[-->
                    <div class="body1-regular-medium mb-[10px] last:mb-0">
                      <div class="gray-999--text">작품명</div>
                      <div class="whitespace-pre-line">
                        강아지 인식표 고양이 목걸이 패브릭 타입
                      </div>
                    </div>
                    <div class="body1-regular-medium mb-[10px] last:mb-0">
                      <div class="gray-999--text">제조자 / 제조국</div>
                      <div class="whitespace-pre-line">
                        왓츄얼네임 / 대한민국
                      </div>
                    </div>
                  </div>
                </div>
                <hr
                  class="BaseDivider"
                  style="--border-color: #f5f5f5"
                  data-v-6ef4cf18=""
                  data-v-6da505b4=""
                />
                <div data-v-6da505b4="">
                  <div
                    class="BaseSubHeader BaseSubHeader--clickable"
                    style="
                      --sub-header-padding-left: 0;
                      --sub-header-padding-right: 0;
                      --sub-header-min-height: 56;
                    "
                    data-v-b42a95d0=""
                  >
                    <div class="BaseSubHeader__contents" data-v-b42a95d0="">
                      <!--[--><span class="subtitle3_bold_small"
                        >교환 / 환불 / 반품</span
                      >
                    </div>
                    <div class="BaseSubHeader__append" data-v-b42a95d0="">
                      <!--[--><svg
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg"
                        style="
                          width: 20px;
                          height: 20px;
                          opacity: 1;
                          fill: currentColor;
                          --BaseIcon-color: #666666;
                        "
                        class="BaseIcon"
                        data-v-6d2bd019=""
                      >
                        <g clip-path="url(#clip0_124_2949)">
                          <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M17.4697 8.46973L18.5304 9.53039L12.5304 15.5304C12.2641 15.7967 11.8475 15.8209 11.5538 15.603L11.4697 15.5304L5.46973 9.53039L6.53039 8.46973L12.0001 13.9391L17.4697 8.46973Z"
                          ></path>
                        </g>
                        <defs>
                          <clipPath id="clip0_124_2949">
                            <rect width="24" height="24"></rect>
                          </clipPath>
                        </defs></svg
                      ><!--]-->
                    </div>
                  </div>
                  <div
                    style="display: none"
                    class="body1_regular_medium pb-[40px]"
                  >
                    <!--[-->
                    <div
                      class="BaseDecorateText transform-gpu whitespace-pre-line inline"
                      data-v-a1957620=""
                    >
                      <span
                        class="DecorateText--bold DecorateText"
                        style="
                          font-size: 14px;
                          color: #333333;
                          background-color: inherit;
                        "
                        data-v-a1957620=""
                        >[반품/교환 배송비]<br
                      /></span>
                    </div>
                    <div
                      class="BaseDecorateText transform-gpu whitespace-pre-line inline"
                      data-v-a1957620=""
                    >
                      <span
                        class="DecorateText"
                        style="
                          font-size: 14px;
                          color: #333333;
                          background-color: inherit;
                        "
                        data-v-a1957620=""
                        >반품 : 편도 3,000원 / 왕복 6,000원 <br />교환 : 6,000원
                        <br />단순변심 및 고객님의 사유로 교환/반품하는 경우,
                        배송비는 고객님 부담입니다.<br />판매 작가님이 설정한
                        최소구매금액(1만 원)이 있는 경우, 고객님의 사유로
                        교환/반품하여 최소구매금액이 불충족될 시, 왕복 배송비는
                        고객님 부담입니다.<br />교환/반품하는 작품의 수량에
                        따라서 배송비는 추가 발생할 수 있습니다.<br />제주/도서산간
                        지역은 기본 배송비 외 추가 운임이 발생할 수 있습니다.
                        배송지역별 금액이 상이하므로 구매 전, 교환/반품 비용을
                        별도 문의하시기를 바랍니다. <br />교환/반품 조건(택배사,
                        배송비 부담, 반품 주소 등)을 반드시 작가님과 확인하고,
                        교환/반품 절차를 진행해 주시기를 바랍니다.<br /><br /><br
                      /></span>
                    </div>
                    <div
                      class="BaseDecorateText transform-gpu whitespace-pre-line inline"
                      data-v-a1957620=""
                    >
                      <span
                        class="DecorateText--bold DecorateText"
                        style="
                          font-size: 14px;
                          color: #333333;
                          background-color: inherit;
                        "
                        data-v-a1957620=""
                        >[교환/반품이 불가능한 경우]<br
                      /></span>
                    </div>
                    <div
                      class="BaseDecorateText transform-gpu whitespace-pre-line inline"
                      data-v-a1957620=""
                    >
                      <span
                        class="DecorateText"
                        style="
                          font-size: 14px;
                          color: #333333;
                          background-color: inherit;
                        "
                        data-v-a1957620=""
                        >• 반품가능기간(작품 수령후 7일 이내)이 지난 경우<br />•
                        고객님의 주문에 따라 개별적으로 생산되는 작품이 제작에
                        들어간 경우<br />• 고객의 사유로 작품 등이 전부 또는
                        일부가 멸실/훼손된 경우(단지, 작품 확인을 위한 포장 훼손
                        제외)<br />• 고객의 사용, 시간 경과, 소비에 의해 작품
                        등의 가치가 현저히 감소한 경우<br />• 시간의 경과에 의해
                        재판매가 곤란할 정도로 작품 등의 가치가 현저히 감소한
                        경우(신선 식품 등)<br />• 복제가 가능한 작품 등의 포장을
                        훼손되거나 디지털 콘텐츠 등의 제공이 개시된 경우<br /><br /><br
                      /></span>
                    </div>
                    <div
                      class="BaseDecorateText transform-gpu whitespace-pre-line inline"
                      data-v-a1957620=""
                    >
                      <span
                        class="DecorateText--bold DecorateText"
                        style="
                          font-size: 14px;
                          color: #333333;
                          background-color: inherit;
                        "
                        data-v-a1957620=""
                        >[작가님 교환/환불 안내]<br
                      /></span>
                    </div>
                    <div
                      class="BaseDecorateText transform-gpu whitespace-pre-line inline"
                      data-v-a1957620=""
                    >
                      <span
                        class="DecorateText"
                        style="
                          font-size: 14px;
                          color: #333333;
                          background-color: inherit;
                        "
                        data-v-a1957620=""
                        >[교환/반품기간]<br />작품 수령일로부터 7일 이내<br />[배송비
                        부담]<br />(고객님 부담) 단순변심, 착오 등 고객님의
                        사유<br />(작가님 부담) 작품 하자 등 작가님의 사유</span
                      >
                    </div>
                    <!--]-->
                  </div>
                </div>
                <hr
                  class="BaseDivider"
                  style="--border-color: #f5f5f5"
                  data-v-6ef4cf18=""
                  data-v-6da505b4=""
                />
                <div data-v-6da505b4="">
                  <div
                    class="BaseSubHeader BaseSubHeader--clickable"
                    style="
                      --sub-header-padding-left: 0;
                      --sub-header-padding-right: 0;
                      --sub-header-min-height: 56;
                    "
                    data-v-b42a95d0=""
                  >
                    <!---->
                    <div class="BaseSubHeader__contents" data-v-b42a95d0="">
                      <!--[--><span class="subtitle3-bold-small"
                        >판매 작가 정보</span
                      ><!--]-->
                    </div>
                    <div class="BaseSubHeader__append" data-v-b42a95d0="">
                      <!--[--><svg
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg"
                        style="
                          width: 20px;
                          height: 20px;
                          opacity: 1;
                          fill: currentColor;
                          --BaseIcon-color: #666666;
                        "
                        class="BaseIcon"
                        data-v-6d2bd019=""
                      >
                        <g clip-path="url(#clip0_124_2949)">
                          <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M17.4697 8.46973L18.5304 9.53039L12.5304 15.5304C12.2641 15.7967 11.8475 15.8209 11.5538 15.603L11.4697 15.5304L5.46973 9.53039L6.53039 8.46973L12.0001 13.9391L17.4697 8.46973Z"
                          ></path>
                        </g>
                        <defs>
                          <clipPath id="clip0_124_2949">
                            <rect width="24" height="24"></rect>
                          </clipPath>
                        </defs></svg
                      ><!--]-->
                    </div>
                  </div>
                  <div
                    style="display: none"
                    class="body1-regular-small pb-[16px]"
                  >
                    <div class="body1-regular-medium mb-[10px] last:mb-0">
                      <div class="gray-999--text">상호</div>
                      <div class="whitespace-pre-line">푸른달 하루</div>
                    </div>
                    <div class="body1-regular-medium mb-[10px] last:mb-0">
                      <div class="gray-999--text">대표자명</div>
                      <div class="whitespace-pre-line">박소형</div>
                    </div>
                    <div class="body1-regular-medium mb-[10px] last:mb-0">
                      <div class="gray-999--text">주소</div>
                      <div class="whitespace-pre-line">
                        경기 화성시 봉담읍 상리3길 160 (수영리, 중흥에스클래스
                        더 센트럴) 209동 2302호
                      </div>
                    </div>
                    <div class="body1-regular-medium mb-[10px] last:mb-0">
                      <div class="gray-999--text">사업자등록번호</div>
                      <div class="whitespace-pre-line">241-71-00431</div>
                    </div>
                    <div class="body1-regular-medium mb-[10px] last:mb-0">
                      <div class="gray-999--text">통신판매업번호</div>
                      <div class="whitespace-pre-line"></div>
                    </div>
                    <div class="body1-regular-medium mb-[10px] last:mb-0">
                      <div class="gray-999--text">이메일</div>
                      <div class="whitespace-pre-line">
                        puleundal01@naver.com
                      </div>
                    </div>
                    <div class="body1-regular-medium mb-[10px] last:mb-0">
                      <div class="gray-999--text">전화번호</div>
                      <div class="whitespace-pre-line">
                        아이디어스 고객센터 1668-3651 작품 관련 문의는 작품
                        상세페이지 내, 우측 하단 “작품 문의” 버튼을 이용해
                        주세요.
                      </div>
                    </div>
                  </div>
                </div>
                <hr
                  class="BaseDivider"
                  style="--border-color: #f5f5f5"
                  data-v-6ef4cf18=""
                  data-v-6da505b4=""
                />
              </div>
              <div id="REVIEW" data-v-6da505b4=""></div>
              <hr
                class="BaseDivider my-[36px]"
                style="--border-color: #d9d9d9"
                data-v-6ef4cf18=""
                data-v-6da505b4=""
              />
              <div data-v-6da505b4="">
                <div
                  class="BaseSubHeader"
                  style="
                    --sub-header-padding-left: 0;
                    --sub-header-padding-right: 0;
                    --sub-header-min-height: 56;
                  "
                  data-v-b42a95d0=""
                >
                  <!---->
                  <div class="BaseSubHeader__contents" data-v-b42a95d0="">
                    <!--[--><span class="subtitle1_bold_small">작품 키워드</span
                    ><!--]-->
                  </div>
                  <!---->
                </div>
                <div class="flex flex-wrap pb-[24px] w-full">
                  <!--[-->
                    <span
                    class="BaseChip__outline BaseChip--medium BaseChip__outline--gray-666 BaseChip mb-[8px] mr-[8px]"
                    data-v-43db7e7b=""
                    v-for="(hashtag, index) in productStore.hashtag" :key="index"
                    >
                        <div
                        class="BaseBadgeNotification__wrapper"
                        data-v-cdfdef93=""
                        data-v-43db7e7b=""
                        >
                        <!--[-->
                            <div
                                class="BaseFontVariable"
                                data-v-9dbc8be1=""
                                data-v-43db7e7b=""
                            >
                                <div class="BaseFontVariable__text" data-v-9dbc8be1="">
                                <span
                                    class=""
                                    data-v-9dbc8be1=""
                                    >#{{hashtag}}</span
                                >
                                </div>
                            </div>
                        </div>
                    </span>
                </div>
              </div>
              <!--]-->
              <hr
                class="BaseDivider my-[36px]"
                style="--border-color: #d9d9d9"
                data-v-6ef4cf18=""
                data-v-6da505b4=""
              />
              <div id="COMMENT" data-v-6da505b4=""></div>
              <!---->
              <div id="ARTIST" data-v-6da505b4="">
                <hr
                  class="BaseDivider my-[36px]"
                  style="--border-color: #d9d9d9"
                  data-v-6ef4cf18=""
                  data-v-6da505b4=""
                />
                <div data-v-6da505b4="" class="flex flex-col w-full">
                  <div class="flex w-full">
                    <a
                      href="https://www.idus.com/w/artist/5697b8a5-24fa-4ec9-87e1-feb61bc66e60"
                      rel="noopener noreferrer"
                      ><div
                        data-v-4bef34a0=""
                        class="BaseAvatarArtist mt-[10px]"
                        style="--overlay-size: 95; --overlay-image: url()"
                      >
                        <div
                          data-v-2fc5c54e=""
                          data-v-4bef34a0=""
                          class="BaseAvatar BaseAvatar--border"
                          style="
                            --BaseAvatar-image: url(//image.idus.com/image/files/3c6ef38e01f34c2eb874f48946bf729d_320.jpg);
                            --BaseAvatar-size: 68;
                          "
                        >
                          <div data-v-2fc5c54e="" class="BaseAvatar__avatar">
                            <div
                              data-v-2fc5c54e=""
                              class="BaseAvatar__avatarImage"
                            ></div>
                          </div>
                          <div
                            data-v-8f163f81=""
                            data-v-2fc5c54e=""
                            class="CoreBadge BaseAvatar__iconDot"
                            style="
                              --CoreBadge-color-background: #ef7014;
                              --CoreBadge-color-outline: #ffffff;
                              --CoreBadge-size: 6;
                              --CoreBadge-fontSize: 11;
                              --CoreBadge-offset-x: 0;
                              --CoreBadge-offset-y: 0;
                              --CoreBadge-position: relative;
                            "
                          >
                            <!---->
                          </div>
                          <div
                            data-v-8f163f81=""
                            data-v-2fc5c54e=""
                            class="CoreBadge CoreBadge__position--bottom BaseAvatar__iconCamera"
                            style="
                              --CoreBadge-color-background: #000000;
                              --CoreBadge-color-outline: #ffffff;
                              --CoreBadge-size: 24;
                              --CoreBadge-fontSize: 11;
                              --CoreBadge-offset-x: 0;
                              --CoreBadge-offset-y: 0;
                              --CoreBadge-position: relative;
                            "
                          >
                            <!---->
                          </div>
                        </div>
                        <div
                          data-v-4bef34a0=""
                          class="BaseAvatarArtist__overlay"
                        ></div></div
                    ></a>
                    <div
                      class="flex flex-col items-start justify-start w-full ml-[8px] mt-[24px]"
                    >
                      <a
                        href="https://www.idus.com/w/artist/5697b8a5-24fa-4ec9-87e1-feb61bc66e60"
                        rel="noopener noreferrer"
                        class="flex items-center"
                        ><span
                          class="subtitle3_bold_small gray-333--text line-clamp-1"
                          >왓츄얼네임 펫 인식표</span
                        ><svg
                          data-v-6d2bd019=""
                          width="24"
                          height="24"
                          viewBox="0 0 24 24"
                          xmlns="http://www.w3.org/2000/svg"
                          class="BaseIcon"
                          style="
                            width: 20px;
                            height: 20px;
                            opacity: 1;
                            fill: currentcolor;
                            --BaseIcon-color: #333333;
                          "
                        >
                          <g clip-path="url(#clip0_124_2947)">
                            <path
                              fill-rule="evenodd"
                              clip-rule="evenodd"
                              d="M9.53039 5.46973L15.5304 11.4697C15.7967 11.736 15.8209 12.1527 15.603 12.4463L15.5304 12.5304L9.53039 18.5304L8.46973 17.4697L13.9391 12.0001L8.46973 6.53039L9.53039 5.46973Z"
                            ></path>
                          </g>
                          <defs>
                            <clipPath id="clip0_124_2947">
                              <rect width="24" height="24"></rect>
                            </clipPath>
                          </defs></svg
                      ></a>
                      <div
                        data-v-2c82c531=""
                        class="BaseRating BaseRating__sizeText--medium mt-[6px]"
                        style="--BaseRating-gap: 0"
                      >
                        <div data-v-2c82c531="" class="BaseRating__rangeList">
                          <svg
                            data-v-6d2bd019=""
                            data-v-2c82c531=""
                            width="24"
                            height="24"
                            viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg"
                            class="BaseIcon CoreStar__type--full BaseRating__rangeListItem"
                            style="
                              width: 14px;
                              height: 14px;
                              opacity: 1;
                              fill: currentcolor;
                              --BaseIcon-color: #ffaf00;
                            "
                          >
                            <g clip-path="url(#clip0_124_2995)">
                              <path
                                fill-rule="evenodd"
                                clip-rule="evenodd"
                                d="M12.4363 2.29343C12.6335 2.39076 12.7931 2.55039 12.8904 2.7476L15.2877 7.60507C15.4334 7.90021 15.715 8.10478 16.0407 8.15211L21.4012 8.93104C21.9478 9.01046 22.3264 9.5179 22.247 10.0644C22.2154 10.2821 22.1129 10.4832 21.9554 10.6367L18.0765 14.4177C17.8408 14.6475 17.7333 14.9785 17.7889 15.3029L18.7046 20.6418C18.798 21.1861 18.4324 21.703 17.888 21.7964C17.6713 21.8336 17.4483 21.7983 17.2536 21.6959L12.459 19.1753C12.1677 19.0221 11.8197 19.0221 11.5284 19.1753L6.73375 21.6959C6.2449 21.9529 5.64028 21.765 5.38327 21.2761C5.28094 21.0815 5.24562 20.8585 5.2828 20.6418L6.19849 15.3029C6.25412 14.9785 6.14658 14.6475 5.91089 14.4177L2.03197 10.6367C1.63649 10.2512 1.62839 9.61812 2.01389 9.22264C2.1674 9.06515 2.36855 8.96267 2.58618 8.93104L7.94672 8.15211C8.27243 8.10478 8.554 7.90021 8.69966 7.60507L11.097 2.7476C11.3414 2.25234 11.941 2.04901 12.4363 2.29343Z"
                              ></path>
                            </g>
                            <defs>
                              <clipPath id="clip0_124_2995">
                                <rect width="24" height="24"></rect>
                              </clipPath>
                            </defs></svg
                          ><svg
                            data-v-6d2bd019=""
                            data-v-2c82c531=""
                            width="24"
                            height="24"
                            viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg"
                            class="BaseIcon CoreStar__type--full BaseRating__rangeListItem"
                            style="
                              width: 14px;
                              height: 14px;
                              opacity: 1;
                              fill: currentcolor;
                              --BaseIcon-color: #ffaf00;
                            "
                          >
                            <g clip-path="url(#clip0_124_2995)">
                              <path
                                fill-rule="evenodd"
                                clip-rule="evenodd"
                                d="M12.4363 2.29343C12.6335 2.39076 12.7931 2.55039 12.8904 2.7476L15.2877 7.60507C15.4334 7.90021 15.715 8.10478 16.0407 8.15211L21.4012 8.93104C21.9478 9.01046 22.3264 9.5179 22.247 10.0644C22.2154 10.2821 22.1129 10.4832 21.9554 10.6367L18.0765 14.4177C17.8408 14.6475 17.7333 14.9785 17.7889 15.3029L18.7046 20.6418C18.798 21.1861 18.4324 21.703 17.888 21.7964C17.6713 21.8336 17.4483 21.7983 17.2536 21.6959L12.459 19.1753C12.1677 19.0221 11.8197 19.0221 11.5284 19.1753L6.73375 21.6959C6.2449 21.9529 5.64028 21.765 5.38327 21.2761C5.28094 21.0815 5.24562 20.8585 5.2828 20.6418L6.19849 15.3029C6.25412 14.9785 6.14658 14.6475 5.91089 14.4177L2.03197 10.6367C1.63649 10.2512 1.62839 9.61812 2.01389 9.22264C2.1674 9.06515 2.36855 8.96267 2.58618 8.93104L7.94672 8.15211C8.27243 8.10478 8.554 7.90021 8.69966 7.60507L11.097 2.7476C11.3414 2.25234 11.941 2.04901 12.4363 2.29343Z"
                              ></path>
                            </g>
                            <defs>
                              <clipPath id="clip0_124_2995">
                                <rect width="24" height="24"></rect>
                              </clipPath>
                            </defs></svg
                          ><svg
                            data-v-6d2bd019=""
                            data-v-2c82c531=""
                            width="24"
                            height="24"
                            viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg"
                            class="BaseIcon CoreStar__type--full BaseRating__rangeListItem"
                            style="
                              width: 14px;
                              height: 14px;
                              opacity: 1;
                              fill: currentcolor;
                              --BaseIcon-color: #ffaf00;
                            "
                          >
                            <g clip-path="url(#clip0_124_2995)">
                              <path
                                fill-rule="evenodd"
                                clip-rule="evenodd"
                                d="M12.4363 2.29343C12.6335 2.39076 12.7931 2.55039 12.8904 2.7476L15.2877 7.60507C15.4334 7.90021 15.715 8.10478 16.0407 8.15211L21.4012 8.93104C21.9478 9.01046 22.3264 9.5179 22.247 10.0644C22.2154 10.2821 22.1129 10.4832 21.9554 10.6367L18.0765 14.4177C17.8408 14.6475 17.7333 14.9785 17.7889 15.3029L18.7046 20.6418C18.798 21.1861 18.4324 21.703 17.888 21.7964C17.6713 21.8336 17.4483 21.7983 17.2536 21.6959L12.459 19.1753C12.1677 19.0221 11.8197 19.0221 11.5284 19.1753L6.73375 21.6959C6.2449 21.9529 5.64028 21.765 5.38327 21.2761C5.28094 21.0815 5.24562 20.8585 5.2828 20.6418L6.19849 15.3029C6.25412 14.9785 6.14658 14.6475 5.91089 14.4177L2.03197 10.6367C1.63649 10.2512 1.62839 9.61812 2.01389 9.22264C2.1674 9.06515 2.36855 8.96267 2.58618 8.93104L7.94672 8.15211C8.27243 8.10478 8.554 7.90021 8.69966 7.60507L11.097 2.7476C11.3414 2.25234 11.941 2.04901 12.4363 2.29343Z"
                              ></path>
                            </g>
                            <defs>
                              <clipPath id="clip0_124_2995">
                                <rect width="24" height="24"></rect>
                              </clipPath>
                            </defs></svg
                          ><svg
                            data-v-6d2bd019=""
                            data-v-2c82c531=""
                            width="24"
                            height="24"
                            viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg"
                            class="BaseIcon CoreStar__type--full BaseRating__rangeListItem"
                            style="
                              width: 14px;
                              height: 14px;
                              opacity: 1;
                              fill: currentcolor;
                              --BaseIcon-color: #ffaf00;
                            "
                          >
                            <g clip-path="url(#clip0_124_2995)">
                              <path
                                fill-rule="evenodd"
                                clip-rule="evenodd"
                                d="M12.4363 2.29343C12.6335 2.39076 12.7931 2.55039 12.8904 2.7476L15.2877 7.60507C15.4334 7.90021 15.715 8.10478 16.0407 8.15211L21.4012 8.93104C21.9478 9.01046 22.3264 9.5179 22.247 10.0644C22.2154 10.2821 22.1129 10.4832 21.9554 10.6367L18.0765 14.4177C17.8408 14.6475 17.7333 14.9785 17.7889 15.3029L18.7046 20.6418C18.798 21.1861 18.4324 21.703 17.888 21.7964C17.6713 21.8336 17.4483 21.7983 17.2536 21.6959L12.459 19.1753C12.1677 19.0221 11.8197 19.0221 11.5284 19.1753L6.73375 21.6959C6.2449 21.9529 5.64028 21.765 5.38327 21.2761C5.28094 21.0815 5.24562 20.8585 5.2828 20.6418L6.19849 15.3029C6.25412 14.9785 6.14658 14.6475 5.91089 14.4177L2.03197 10.6367C1.63649 10.2512 1.62839 9.61812 2.01389 9.22264C2.1674 9.06515 2.36855 8.96267 2.58618 8.93104L7.94672 8.15211C8.27243 8.10478 8.554 7.90021 8.69966 7.60507L11.097 2.7476C11.3414 2.25234 11.941 2.04901 12.4363 2.29343Z"
                              ></path>
                            </g>
                            <defs>
                              <clipPath id="clip0_124_2995">
                                <rect width="24" height="24"></rect>
                              </clipPath>
                            </defs></svg
                          ><svg
                            data-v-6d2bd019=""
                            data-v-2c82c531=""
                            width="24"
                            height="24"
                            viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg"
                            class="BaseIcon CoreStar__type--full BaseRating__rangeListItem"
                            style="
                              width: 14px;
                              height: 14px;
                              opacity: 1;
                              fill: currentcolor;
                              --BaseIcon-color: #ffaf00;
                            "
                          >
                            <g clip-path="url(#clip0_124_2995)">
                              <path
                                fill-rule="evenodd"
                                clip-rule="evenodd"
                                d="M12.4363 2.29343C12.6335 2.39076 12.7931 2.55039 12.8904 2.7476L15.2877 7.60507C15.4334 7.90021 15.715 8.10478 16.0407 8.15211L21.4012 8.93104C21.9478 9.01046 22.3264 9.5179 22.247 10.0644C22.2154 10.2821 22.1129 10.4832 21.9554 10.6367L18.0765 14.4177C17.8408 14.6475 17.7333 14.9785 17.7889 15.3029L18.7046 20.6418C18.798 21.1861 18.4324 21.703 17.888 21.7964C17.6713 21.8336 17.4483 21.7983 17.2536 21.6959L12.459 19.1753C12.1677 19.0221 11.8197 19.0221 11.5284 19.1753L6.73375 21.6959C6.2449 21.9529 5.64028 21.765 5.38327 21.2761C5.28094 21.0815 5.24562 20.8585 5.2828 20.6418L6.19849 15.3029C6.25412 14.9785 6.14658 14.6475 5.91089 14.4177L2.03197 10.6367C1.63649 10.2512 1.62839 9.61812 2.01389 9.22264C2.1674 9.06515 2.36855 8.96267 2.58618 8.93104L7.94672 8.15211C8.27243 8.10478 8.554 7.90021 8.69966 7.60507L11.097 2.7476C11.3414 2.25234 11.941 2.04901 12.4363 2.29343Z"
                              ></path>
                            </g>
                            <defs>
                              <clipPath id="clip0_124_2995">
                                <rect width="24" height="24"></rect>
                              </clipPath>
                            </defs>
                          </svg>
                        </div>
                        <div data-v-2c82c531="" class="BaseRating__label">
                          <div data-v-2c82c531="" class="BaseRating__labelRate">
                            5.0
                          </div>
                          <div
                            data-v-2c82c531=""
                            class="BaseRating__labelAppned"
                          >
                            (238)
                          </div>
                        </div>
                      </div>
                      <div class="flex w-full mt-[16px] text-left">
                        <div class="flex flex-col w-full">
                          <div class="body1_regular_small gray-999--text">
                            작품 찜
                          </div>
                          <div class="subtitle1_bold_small mt-[4px]">731</div>
                        </div>
                        <div class="flex flex-col w-full">
                          <div class="body1_regular_small gray-999--text">
                            팔로워
                          </div>
                          <div class="subtitle1_bold_small mt-[4px]">25</div>
                        </div>
                        <div class="flex flex-col w-full">
                          <div class="body1_regular_small gray-999--text">
                            후원해주신 분들
                          </div>
                          <div class="subtitle1_bold_small mt-[4px]">48</div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div>
                    <div
                      data-v-fbfd1092=""
                      class="ArtistIntroduce my-[24px]"
                      style="--introduce-height: 36px"
                    >
                      <div data-v-fbfd1092="" class="ArtistIntroduce__inner text-left">
                        핸드메이드 자체제작 패브릭 끈으로 만든 펫 인식표 제품을
                        판매합니다. 신제품 계속 업로드하니 많이 많이 구경오세요
                        ^^♡
                      </div>
                      <!---->
                    </div>
                    <div class="grid grid-cols-2 gap-x-[4px] pb-[16px]">
                      <div
                        data-v-b6faa6c8=""
                        class="IDSTooltip__wrapper IDSTooltip__display--hide IDSTooltip__colorType--yellow IDSTooltip__display--blocked IDSTooltip__innerPosition--bottom IDSTooltip__innerAlign--center IDSTooltip__innerCloseAlign--center"
                        style="--ids-tooltip-full-left: --8px"
                      >
                        <div data-v-b6faa6c8="" class="IDSTooltip__trigger">
                          <button
                            data-v-524f63ea=""
                            data-v-7940d6dd=""
                            type="outline"
                            class="CoreButton CoreButton--block BaseButtonRectangle body1-regular-small BaseButtonRectangle__outline"
                            style="
                              background-color: rgb(255, 255, 255);
                              color: rgb(239, 112, 20);
                              height: 36px;
                              flex-direction: row;
                              --core-button-padding-x: 8;
                              --button-rectangle-border-color: #ef7014;
                            "
                          >
                            <!----><svg
                              data-v-6d2bd019=""
                              data-v-524f63ea=""
                              width="24"
                              height="24"
                              viewBox="0 0 24 24"
                              xmlns="http://www.w3.org/2000/svg"
                              class="BaseIcon CoreButton__icon"
                              style="
                                width: 18px;
                                height: 18px;
                                opacity: 1;
                                fill: currentcolor;
                                --BaseIcon-color: #333333;
                                margin-right: 2px;
                              "
                            >
                              <g clip-path="url(#clip0_124_2960)">
                                <path
                                  fill-rule="evenodd"
                                  clip-rule="evenodd"
                                  d="M12.75 5V11.25H19V12.75H12.75V19H11.25V12.75H5V11.25H11.25V5H12.75Z"
                                ></path>
                              </g>
                              <defs>
                                <clipPath id="clip0_124_2960">
                                  <rect width="24" height="24"></rect>
                                </clipPath>
                              </defs>
                            </svg>
                            <div
                              data-v-524f63ea=""
                              class="inline-flex items-center"
                            >
                              <span data-v-524f63ea="" class="CoreButton__text"
                                >팔로우</span
                              >
                            </div>
                          </button>
                        </div>
                        <!---->
                      </div>
                     <button
                      data-v-524f63ea=""
                      data-v-7940d6dd=""
                      data-v-9937b295=""
                      type="outline"
                      class="CoreButton BaseButtonRectangle body1-regular-small BaseButtonRectangle__outline"
                      style="
                        background-color: rgb(255, 255, 255);
                        color: rgb(51, 51, 51);
                        flex-direction: row;
                        --core-button-padding-x: 8;
                        font-weight: bold;
                        --button-rectangle-border-color: #acacac;
                      "
                    >
                      <!----><!---->
                      <div data-v-524f63ea="" class="inline-flex items-center">
                        <span data-v-524f63ea="" class="CoreButton__text"
                          >작가님의 다른 작품 더보기</span
                        >
                      </div>
                    </button>
                    </div>
                    <!---->
                  </div>
                  <!---->
                </div>
              </div>
            </div>
          </div>
          <!----><!----><!----><!----><!---->
          <div class="TheFabContainer" data-v-6da505b4="" data-v-0397dfb3="">
            <!--[--><button
              type="button"
              class="CoreButton IdsButtonFab__size--medium IdsButtonFab__iconSize--medium IdsButtonFab TheMoveToTopFab__display--hide mb-[12px]"
              style="
                background-color: rgb(255, 255, 255);
                color: rgb(51, 51, 51);
                height: 56px;
                width: 56px;
                flex-direction: row;
                --core-button-padding-x: 0px;
              "
              data-v-6da505b4=""
              data-v-524f63ea=""
              data-v-0b9311f9=""
              data-v-01e4f5dd=""
            >
              <!----><!--[-->
              <div class="IdsButtonFab__contents" data-v-0b9311f9="">
                <div class="IdsButtonFab__contentsInner" data-v-0b9311f9="">
                  <!--[-->
                  <div data-v-0b9311f9="">
                    <svg
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      xmlns="http://www.w3.org/2000/svg"
                      style="
                        width: 32px;
                        height: 32px;
                        opacity: 1;
                        fill: currentColor;
                        --BaseIcon-color: #333333;
                      "
                      class="BaseIcon IdsButtonFab__contentsIcon"
                      data-v-6d2bd019=""
                      data-v-0b9311f9=""
                    >
                      <g clip-path="url(#clip0_124_2944)">
                        <path
                          d="M3.96869 11.7197L11.4687 4.21967C11.7616 3.92678 12.2365 3.92678 12.5293 4.21967L20.0293 11.7197L18.9687 12.7803L12.749 6.5606V20H11.249V6.56072L5.02935 12.7803L3.96869 11.7197Z"
                        ></path>
                      </g>
                      <defs>
                        <clipPath id="clip0_124_2944">
                          <rect width="24" height="24"></rect>
                        </clipPath>
                      </defs>
                    </svg>
                  </div>
                  <!--]--><!---->
                </div>
              </div>
              <!--]--></button
            ><!--]-->
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import CarouselProductDetailComponent from '@/components/product/CarouselProductDetailComponent.vue'
import ProductDetailPaymentComponent from '@/components/product/ProductDetailPaymentComponent.vue'

import { mapStores } from "pinia";
import { useProductStore } from "@/stores/useProductStore";

export default {
    computed:{
        ...mapStores(useProductStore)
    },
    components:{
        CarouselProductDetailComponent,
        ProductDetailPaymentComponent
    },
    data(){
        return{
            pagenation:[
                {idx : 1, name : "작품정보"}, {idx : 2, name : "후기", count : 238}, {idx : 3, name : "문의"}, {idx : 4, name : "공빙 정보"}
            ]
        }
    }
};
</script>

<style scoped>
.text-left{
    text-align: left !important;
}
</style>