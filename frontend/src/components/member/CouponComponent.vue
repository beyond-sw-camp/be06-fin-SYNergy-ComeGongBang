<template>
  <div id="__nuxt">
    <div>
      <!--[-->
      <div>
        <div
          class="appContainer"
          style="
            --desktop-header-menu-top: 75px;
            --desktop-header-sticky-height: 124px;
          "
          data-v-261d543d=""
        >
          <div class="appContents" data-v-261d543d="">
            <!--[-->
            <div class="flex mt-[36px] mb-[64px]">
              <div class="MyPageSideBar">
                <div class="MyPageSidebarDesktop w-[180px]" data-v-095dc081="">
                  <div
                    class="MyPageProfileDesktop h-fit flex flex-col items-center py-[28px] border-t-[1px] border-x-[1px] border-b-[0] border-[#ff7b30]"
                    data-v-095dc081=""
                  >
                    <!--[--><label
                      for="profileImageInput"
                      class="MyProfileImageUploader group"
                      style="
                        --profile-image: url(https://image.idus.com/image/files/b5b721bcb181484797e726ba90503777.jpg);
                      "
                      data-v-b4eb4e33=""
                      ><div
                        class="MyProfileImageUploader__image"
                        data-v-b4eb4e33=""
                      ></div>
                      <input
                        id="profileImageInput"
                        hidden=""
                        type="file"
                        name="image"
                        accept=".png, .jpg, .jpeg"
                        value=""
                        data-v-b4eb4e33=""
                      /><span
                        class="MyProfileImageUploader__edit group-hover:inline-block"
                        for="profileImageInput"
                        data-v-b4eb4e33=""
                        >수정</span
                      ></label
                    ><!----><!--]-->
                    <div class="flex justify-center align-middle mt-[20px]">
                      <img
                        src="https://cdn.idus.kr/static/common/images/20240524/073952_image_grade01.png"
                        width="24"
                        height="24"
                        class="flex my-auto"
                      /><button
                        type="default"
                        class="CoreButton BaseButtonText body1-regular-small !pr-0"
                        style="
                          background-color: transparent;
                          color: black;
                          height: 32px;
                          flex-direction: row-reverse;
                          --core-button-padding-x: 8;
                          font-weight: bold;
                        "
                        data-v-524f63ea=""
                        data-v-8493c3f2=""
                      >
                        <!----><!--[--><svg
                          width="24"
                          height="24"
                          viewBox="0 0 24 24"
                          xmlns="http://www.w3.org/2000/svg"
                          style="
                            width: 18px;
                            height: 18px;
                            opacity: 1;
                            fill: currentColor;
                            --BaseIcon-color: #333333;
                            margin-left: 0px;
                          "
                          class="BaseIcon CoreButton__icon"
                          data-v-6d2bd019=""
                          data-v-524f63ea=""
                        >
                          <g clip-path="url(#clip0_124_2947)">
                            <path
                              fill-rule="evenodd"
                              clip-rule="evenodd"
                              d="M9.53039 5.46973L15.5304 11.4697C15.7967 11.736 15.8209 12.1527 15.603 12.4463L15.5304 12.5304L9.53039 18.5304L8.46973 17.4697L13.9391 12.0001L8.46973 6.53039L9.53039 5.46973Z"
                            ></path>
                          </g>
                          <defs>
                            <clipPath id="clip0_124_2947">
                              <rect width="24" height="24"></rect>
                            </clipPath>
                          </defs>
                        </svg>
                        <div
                          class="inline-flex items-center"
                          data-v-524f63ea=""
                        >
                          <!--[--><!--]--><span
                            data-v-524f63ea=""
                            class="CoreButton__text"
                            >아기손</span
                          >
                        </div>
                        <!--]-->
                      </button>
                    </div>
                    <p
                      class="body1-bold-small text-center my-[10px] break-all w-[140px] text-ellipsis overflow-hidden whitespace-nowrap"
                    >
                      김동욱
                    </p>
                    <!---->
                  </div>
                  <p
                    class="block px-[16px] py-[9px] text-white bg-[#ff7b30] body1-bold-medium"
                    data-v-095dc081=""
                  >
                    MY MENU
                  </p>
                  <nav class="bg-[#fbfafa]" data-v-095dc081="">
                    <!--[-->
                    <div
                      class="border-solid border-b border-[#efe8e6] last:border-b-0 py-[11px] px-[16px]"
                      data-v-095dc081=""
                    >
                      <span class="body2-bold-medium" data-v-095dc081=""
                        >주문 배송</span
                      >
                      <div data-v-095dc081="">
                        <!--[-->
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/order"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >주문/배송 내역</a
                          >
                        </div>
                        <!--]-->
                      </div>
                    </div>
                    <div
                      class="border-solid border-b border-[#efe8e6] last:border-b-0 py-[11px] px-[16px]"
                      data-v-095dc081=""
                    >
                      <span class="body2-bold-medium" data-v-095dc081=""
                        >알림 및 메시지</span
                      >
                      <div data-v-095dc081="">
                        <!--[-->
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/notification"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >알림</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="https://www.idus.com/w/message"
                            rel="noopener noreferrer"
                            target="_self"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >메시지</a
                          >
                        </div>
                        <!--]-->
                      </div>
                    </div>
                    <div
                      class="border-solid border-b border-[#efe8e6] last:border-b-0 py-[11px] px-[16px]"
                      data-v-095dc081=""
                    >
                      <span class="body2-bold-medium" data-v-095dc081=""
                        >선물함</span
                      >
                      <div data-v-095dc081="">
                        <!--[-->
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/gift/receive"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >받은 선물함</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/gift/send"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >보낸 선물함</a
                          >
                        </div>
                        <!--]-->
                      </div>
                    </div>
                    <div
                      class="border-solid border-b border-[#efe8e6] last:border-b-0 py-[11px] px-[16px]"
                      data-v-095dc081=""
                    >
                      <span class="body2-bold-medium" data-v-095dc081=""
                        >기프트카드함</span
                      >
                      <div data-v-095dc081="">
                        <!--[-->
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/gift-card/receive"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >받은 카드함</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/gift-card/send"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >보낸 카드함</a
                          >
                        </div>
                        <!--]-->
                      </div>
                    </div>
                    <div
                      class="border-solid border-b border-[#efe8e6] last:border-b-0 py-[11px] px-[16px]"
                      data-v-095dc081=""
                    >
                      <span class="body2-bold-medium" data-v-095dc081=""
                        >나의 구매후기</span
                      >
                      <div data-v-095dc081="">
                        <!--[-->
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/review/writable"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >작성 가능한 후기</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/review/written"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >작성한 후기</a
                          >
                        </div>
                        <!--]-->
                      </div>
                    </div>
                    <div
                      class="border-solid border-b border-[#efe8e6] last:border-b-0 py-[11px] px-[16px]"
                      data-v-095dc081=""
                    >
                      <span class="body2-bold-medium" data-v-095dc081=""
                        >관심</span
                      >
                      <div data-v-095dc081="">
                        <!--[-->
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/favorite/favorite-list"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >찜한 작품</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/favorite/follow-artist"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >팔로우하는 작가</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/favorite/recently-view"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >최근 본 작품</a
                          >
                        </div>
                        <!--]-->
                      </div>
                    </div>
                    <div
                      class="border-solid border-b border-[#efe8e6] last:border-b-0 py-[11px] px-[16px]"
                      data-v-095dc081=""
                    >
                      <span class="body2-bold-medium" data-v-095dc081=""
                        >할인혜택</span
                      >
                      <div data-v-095dc081="">
                        <!--[-->
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/membership"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >d+ 멤버십</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/event/point-payback"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >idus share 적립금</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            aria-current="page"
                            href="/v2/mypage/coupon"
                            class="router-link-active router-link-exact-active body3-regular-small MyPageSidebarDesktop__menu activate"
                            data-v-095dc081=""
                            >쿠폰함</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/point"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >적립금</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/gift-card-point"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >기프트카드 포인트</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/invitation"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >친구 초대 쿠폰</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="https://bit.ly/3IDdeyE"
                            rel="noopener noreferrer"
                            target="_blank"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >단체선물 문의</a
                          >
                        </div>
                        <!--]-->
                      </div>
                    </div>
                    <div
                      class="border-solid border-b border-[#efe8e6] last:border-b-0 py-[11px] px-[16px]"
                      data-v-095dc081=""
                    >
                      <span class="body2-bold-medium" data-v-095dc081=""
                        >내 정보</span
                      >
                      <div data-v-095dc081="">
                        <!--[-->
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/grade"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >회원 등급</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/edit-user-info"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >회원 정보 관리</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/payment-management"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >신용카드 및 환불계좌 관리</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="/v2/mypage/delivery"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >배송지 관리</a
                          >
                        </div>
                        <div class="flex flex-col" data-v-095dc081="">
                          <a
                            href="https://www.idus.com/w/authorization"
                            rel="noopener noreferrer"
                            target="_self"
                            class="body3-regular-small MyPageSidebarDesktop__menu"
                            data-v-095dc081=""
                            >본인 인증</a
                          >
                        </div>
                        <!--]-->
                      </div>
                    </div>
                    <!--]-->
                  </nav>
                </div>
              </div>
              <div class="w-full w-full pl-6">
                <!--[-->
                <div
                  class="BaseSubHeader mb-[36px]"
                  style="
                    --sub-header-padding-left: 0;
                    --sub-header-padding-right: 0;
                    --sub-header-min-height: 48;
                  "
                  data-v-b42a95d0=""
                >
                  <!---->
                  <div class="BaseSubHeader__contents" data-v-b42a95d0="">
                    <!--[-->
                    <p class="headline4-bold-small">쿠폰함</p>
                    <!--]-->
                  </div>
                </div>
                <!----><!--]-->
                <div
                  class="BaseTabs BaseTabs__size--medium BaseTabs--full BaseTabs__style--outline BaseTabs__border--bottom BaseTabs__align--left"
                  style="--BaseTabs-item-length: 2; --BaseTabs-margin-x: 0"
                  data-v-de0565e1=""
                >
                  <div
                    class="BaseTabs__inner"
                    role="tablist"
                    data-v-de0565e1=""
                  >
                    <!--[-->
                    <div
                      class="BaseTab BaseTab--active"
                      style="text-align: center"
                      role="tab"
                      border-top="false"
                      border-bottom="true"
                      data-v-47c662bd=""
                      data-v-de0565e1=""
                    >
                      <div class="BaseTab__contents" data-v-47c662bd="">
                        <div class="BaseTab__prefix" data-v-47c662bd="">
                          <!--[--><!--]-->
                        </div>
                        <div
                          class="BaseFontVariable"
                          data-v-9dbc8be1=""
                          data-v-47c662bd=""
                        >
                          <div
                            class="BaseFontVariable__text"
                            data-v-9dbc8be1=""
                          >
                            <span
                              class="BaseFontVariable__text--hidden"
                              data-v-9dbc8be1=""
                              >보유 쿠폰</span
                            ><span
                              class="BaseFontVariable__text--display"
                              data-v-9dbc8be1=""
                              >보유 쿠폰</span
                            >
                          </div>
                          <span
                            class="flex-auto inline-flex items-center"
                            data-v-9dbc8be1=""
                            ><!--[--><!--]--></span
                          >
                        </div>
                      </div>
                      <div class="BaseTab__empty" data-v-47c662bd=""></div>
                      <div class="BaseTab__activator" data-v-47c662bd=""></div>
                    </div>
                    <!--]-->
                  </div>
                  <!---->
                </div>
                <div class="px-[16px] mb-[24px] flex flex-col items-center">
                  <!---->
                  <div class="w-[900px] mt-[36px] mb-[36px]">
                    <p class="mb-[24px] body1-regular-small">
                      <span class="body1-bold-small">3 개</span> 쿠폰
                    </p>
                    <div class="grid grid-cols-2 gap-[12px]">
                      <!--[-->
                      <div
                        class="relative h-[240px] px-[28px] flex items-center"
                      >
                        <img
                          src="data:image/png;base64,"
                          class="absolute top-0 left-0 w-full h-full"
                        />
                        <div class="flex flex-col justify-center">
                          <div class="mb-[16px] flex">
                            <!--[-->
                            <div
                              class="BaseBadgeBusiness BaseBadgeBusiness__colorType--orange-500 BaseBadgeBusiness__size--large mr-[6px]"
                              style="font-weight: normal"
                              data-v-24a9185e=""
                            >
                              <!---->
                              작품
                              <!--[--><!--]-->
                            </div>
                            <!--]-->
                          </div>
                          <div class="flex items-center">
                            <div
                              class="BaseAvatar shrink-0"
                              style="
                                --BaseAvatar-image: url(https://image.idus.com/static/idus_logo_new.png);
                                --BaseAvatar-size: 32;
                              "
                              data-v-2fc5c54e=""
                            >
                              <div
                                class="BaseAvatar__avatar"
                                data-v-2fc5c54e=""
                              >
                                <div
                                  class="BaseAvatar__avatarImage"
                                  data-v-2fc5c54e=""
                                ></div>
                              </div>
                              <div
                                class="CoreBadge BaseAvatar__iconDot"
                                style="
                                  --CoreBadge-color-background: #ef7014;
                                  --CoreBadge-color-outline: #ffffff;
                                  --CoreBadge-size: 6;
                                  --CoreBadge-fontSize: 11;
                                  --CoreBadge-offset-x: 0;
                                  --CoreBadge-offset-y: 0;
                                  --CoreBadge-position: relative;
                                "
                                data-v-8f163f81=""
                                data-v-2fc5c54e=""
                              >
                                <!--[--><!--]--><!---->
                              </div>
                              <div
                                class="CoreBadge CoreBadge__position--bottom BaseAvatar__iconCamera"
                                style="
                                  --CoreBadge-color-background: #000000;
                                  --CoreBadge-color-outline: #ffffff;
                                  --CoreBadge-size: 20;
                                  --CoreBadge-fontSize: 11;
                                  --CoreBadge-offset-x: 0;
                                  --CoreBadge-offset-y: 0;
                                  --CoreBadge-position: relative;
                                "
                                data-v-8f163f81=""
                                data-v-2fc5c54e=""
                              >
                                <!--[--><!--]--><!---->
                              </div>
                            </div>
                            <p class="body1-regular-small ml-[8px]">
                              [당첨] 고객님 단독 첫 구매 할인
                            </p>
                          </div>
                          <div class="flex flex-col pl-[40px] mt-[4px]">
                            <p class="headline3-bold-small h-[32px] shrink-0">
                              3,000원
                            </p>
                            <p
                              class="body3-regular-small gray-999--text mt-[8px]"
                            >
                              10,000원 이상 구매시
                            </p>
                            <div class="flex items-center mt-[8px]">
                              <p
                                class="body3-bold-small orange-500--text mr-[4px]"
                              >
                                1일 남음
                              </p>
                              <p class="body3-regular-small gray-666--text">
                                2024.09.13 ~ 2024.09.19
                              </p>
                            </div>
                            <div class="mt-[16px] flex"><!--[--><!--]--></div>
                          </div>
                        </div>
                      </div>
                      <div
                        class="relative h-[240px] px-[28px] flex items-center"
                      >
                        <img
                          src="data:image/png;base64,"
                          class="absolute top-0 left-0 w-full h-full"
                        />
                        <div class="flex flex-col justify-center">
                          <!---->
                          <div class="flex items-center">
                            <div
                              class="BaseAvatar shrink-0"
                              style="
                                --BaseAvatar-image: url(https://image.idus.com/image/files/18cd542b94ff4dc79ca46f1a21fd7dd2_144.jpg);
                                --BaseAvatar-size: 32;
                              "
                              data-v-2fc5c54e=""
                            >
                              <div
                                class="BaseAvatar__avatar"
                                data-v-2fc5c54e=""
                              >
                                <div
                                  class="BaseAvatar__avatarImage"
                                  data-v-2fc5c54e=""
                                ></div>
                              </div>
                              <div
                                class="CoreBadge BaseAvatar__iconDot"
                                style="
                                  --CoreBadge-color-background: #ef7014;
                                  --CoreBadge-color-outline: #ffffff;
                                  --CoreBadge-size: 6;
                                  --CoreBadge-fontSize: 11;
                                  --CoreBadge-offset-x: 0;
                                  --CoreBadge-offset-y: 0;
                                  --CoreBadge-position: relative;
                                "
                                data-v-8f163f81=""
                                data-v-2fc5c54e=""
                              >
                                <!--[--><!--]--><!---->
                              </div>
                              <div
                                class="CoreBadge CoreBadge__position--bottom BaseAvatar__iconCamera"
                                style="
                                  --CoreBadge-color-background: #000000;
                                  --CoreBadge-color-outline: #ffffff;
                                  --CoreBadge-size: 20;
                                  --CoreBadge-fontSize: 11;
                                  --CoreBadge-offset-x: 0;
                                  --CoreBadge-offset-y: 0;
                                  --CoreBadge-position: relative;
                                "
                                data-v-8f163f81=""
                                data-v-2fc5c54e=""
                              >
                                <!--[--><!--]--><!---->
                              </div>
                            </div>
                            <p class="body1-regular-small ml-[8px]">
                              JJ달콤상회 첫 팔로우 쿠폰
                            </p>
                          </div>
                          <div class="flex flex-col pl-[40px] mt-[4px]">
                            <p class="headline3-bold-small h-[32px] shrink-0">
                              1,000원
                            </p>
                            <p
                              class="body3-regular-small gray-999--text mt-[8px]"
                            >
                              30,000원 이상 구매시
                            </p>
                            <div class="flex items-center mt-[8px]">
                              <p
                                class="body3-bold-small orange-500--text mr-[4px]"
                              >
                                16일 남음
                              </p>
                              <p class="body3-regular-small gray-666--text">
                                2024.09.04 ~ 2024.10.04
                              </p>
                            </div>
                            <div class="mt-[16px] flex">
                              <!--[--><button
                                type="outline"
                                class="CoreButton BaseButtonRectangle body3-regular-small BaseButtonRectangle__outline mr-[8px]"
                                style="
                                  background-color: #ffffff;
                                  color: #333333;
                                  height: 32px;
                                  flex-direction: row;
                                  --core-button-padding-x: 8;
                                  --button-rectangle-border-color: #acacac;
                                "
                                data-v-524f63ea=""
                                data-v-7940d6dd=""
                              >
                                <!----><!--[--><!---->
                                <div
                                  class="inline-flex items-center"
                                  data-v-524f63ea=""
                                >
                                  <!--[--><!--]--><span
                                    class="CoreButton__text"
                                    data-v-524f63ea=""
                                    >적용 가능 작품 보기</span
                                  >
                                </div>
                                <!--]--></button
                              ><button
                                type="outline"
                                class="CoreButton BaseButtonRectangle body3-regular-small BaseButtonRectangle__outline"
                                style="
                                  background-color: #ffffff;
                                  color: #ef7014;
                                  height: 32px;
                                  flex-direction: row;
                                  --core-button-padding-x: 8;
                                  --button-rectangle-border-color: #ef7014;
                                "
                                data-v-524f63ea=""
                                data-v-7940d6dd=""
                              >
                                <!----><!--[--><!---->
                                <div
                                  class="inline-flex items-center"
                                  data-v-524f63ea=""
                                >
                                  <!--[--><!--]--><span
                                    class="CoreButton__text"
                                    data-v-524f63ea=""
                                    >쿠폰 선물하기</span
                                  >
                                </div>
                                <!--]--></button
                              ><!--]-->
                            </div>
                          </div>
                        </div>
                      </div>
                      <!--]-->
                    </div>
                  </div>
                  <div>
                    <div class="BasePagination" data-v-e5a5a8d6="">
                      <button
                        type="button"
                        class="CoreButton CoreButton--disabled BaseButtonIcon BasePagination__arrow mr-[8px]"
                        style="
                          background-color: transparent;
                          color: #000;
                          height: 32px;
                          flex-direction: column;
                          --core-button-padding-x: 4;
                        "
                        data-v-524f63ea=""
                        data-v-778c1d9b=""
                        data-v-e5a5a8d6=""
                      >
                        <!----><!--[--><svg
                          width="24"
                          height="24"
                          viewBox="0 0 24 24"
                          xmlns="http://www.w3.org/2000/svg"
                          style="
                            width: 24px;
                            height: 24px;
                            opacity: 1;
                            fill: currentColor;
                            --BaseIcon-color: #333333;
                            margin-bottom: 0px;
                          "
                          class="BaseIcon CoreButton__icon"
                          data-v-6d2bd019=""
                          data-v-524f63ea=""
                        >
                          <g clip-path="url(#clip0_124_2946)">
                            <path
                              fill-rule="evenodd"
                              clip-rule="evenodd"
                              d="M14.4697 5.46973L15.5303 6.53039L10.061 12.0001L15.5303 17.4697L14.4697 18.5304L8.46967 12.5304C8.2034 12.2641 8.1792 11.8475 8.39705 11.5538L8.46967 11.4697L14.4697 5.46973Z"
                            ></path>
                          </g>
                          <defs>
                            <clipPath id="clip0_124_2946">
                              <rect width="24" height="24"></rect>
                            </clipPath>
                          </defs>
                        </svg>
                        <div
                          class="inline-flex items-center"
                          data-v-524f63ea=""
                        >
                          <!--[--><!--]--><!---->
                        </div>
                        <!--]-->
                      </button>
                      <div class="BasePagination__pages" data-v-e5a5a8d6="">
                        <div
                          class="BasePaginationItem BasePaginationItem--active BasePagination__item"
                          data-v-77e9e007=""
                          data-v-e5a5a8d6=""
                        >
                          <div
                            class="BasePaginationItem__item BasePaginationItem__number"
                            data-v-77e9e007=""
                          >
                            1
                          </div>
                        </div>
                        <!--[--><!--]-->
                        <div
                          class="BasePaginationItem BasePaginationItem--active BasePagination__item"
                          style="display: none"
                          data-v-77e9e007=""
                          data-v-e5a5a8d6=""
                        >
                          <div
                            class="BasePaginationItem__item BasePaginationItem__number"
                            data-v-77e9e007=""
                          >
                            1
                          </div>
                        </div>
                      </div>
                      <button
                        type="button"
                        class="CoreButton CoreButton--disabled BaseButtonIcon BasePagination__arrow ml-[8px]"
                        style="
                          background-color: transparent;
                          color: #000;
                          height: 32px;
                          flex-direction: column;
                          --core-button-padding-x: 4;
                        "
                        data-v-524f63ea=""
                        data-v-778c1d9b=""
                        data-v-e5a5a8d6=""
                      >
                        <!----><!--[--><svg
                          width="24"
                          height="24"
                          viewBox="0 0 24 24"
                          xmlns="http://www.w3.org/2000/svg"
                          style="
                            width: 24px;
                            height: 24px;
                            opacity: 1;
                            fill: currentColor;
                            --BaseIcon-color: #333333;
                            margin-bottom: 0px;
                          "
                          class="BaseIcon CoreButton__icon"
                          data-v-6d2bd019=""
                          data-v-524f63ea=""
                        >
                          <g clip-path="url(#clip0_124_2947)">
                            <path
                              fill-rule="evenodd"
                              clip-rule="evenodd"
                              d="M9.53039 5.46973L15.5304 11.4697C15.7967 11.736 15.8209 12.1527 15.603 12.4463L15.5304 12.5304L9.53039 18.5304L8.46973 17.4697L13.9391 12.0001L8.46973 6.53039L9.53039 5.46973Z"
                            ></path>
                          </g>
                          <defs>
                            <clipPath id="clip0_124_2947">
                              <rect width="24" height="24"></rect>
                            </clipPath>
                          </defs>
                        </svg>
                        <div
                          class="inline-flex items-center"
                          data-v-524f63ea=""
                        >
                          <!--[--><!--]--><!---->
                        </div>
                        <!--]-->
                      </button>
                    </div>
                  </div>
                  <div
                    class="px-[16px] py-[32px] gray-f5--background w-full !px-[32px] mt-[24px]"
                  >
                    <p class="subtitle3-bold-medium gray-666--text mb-[10px]">
                      안내사항
                    </p>
                    <ul class="list-disc">
                      <!--[-->
                      <li
                        class="body3-regular-medium gray-666--text ml-[16px] mb-[4px] last:mb-0"
                      >
                        작가님 발행 쿠폰은 장바구니에서 작가님 별로 1개의 쿠폰만
                        적용 가능합니다.
                      </li>
                      <li
                        class="body3-regular-medium gray-666--text ml-[16px] mb-[4px] last:mb-0"
                      >
                        아이디어스 쿠폰은 주문결제 단계에서 주문 건당 1개의
                        쿠폰만 적용 가능합니다.
                      </li>
                      <li
                        class="body3-regular-medium gray-666--text ml-[16px] mb-[4px] last:mb-0"
                      >
                        쿠폰은 구매하신 작가님 별 구매금액 비율로 최종 할인이
                        적용됩니다.
                      </li>
                      <!--]-->
                    </ul>
                  </div>
                </div>
                <!---->
              </div>
            </div>
            <!--]-->
            <div class="TheFabContainer" data-v-261d543d="" data-v-0397dfb3="">
              <!--[--><!----><button
                type="button"
                class="CoreButton IdsButtonFab__size--medium IdsButtonFab__iconSize--medium IdsButtonFab TheMoveToTopFab__display--hide TheFabContainer__item"
                style="
                  background-color: rgb(255, 255, 255);
                  color: rgb(51, 51, 51);
                  height: 56px;
                  width: 56px;
                  flex-direction: row;
                  --core-button-padding-x: 0px;
                "
                data-v-0397dfb3=""
                data-v-524f63ea=""
                data-v-0b9311f9=""
                data-v-01e4f5dd=""
              >
                <!----><!--[-->
                <div class="IdsButtonFab__contents" data-v-0b9311f9="">
                  <div class="IdsButtonFab__contentsInner" data-v-0b9311f9="">
                    <!--[-->
                    <div data-v-0b9311f9="">
                      <svg
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg"
                        style="
                          width: 32px;
                          height: 32px;
                          opacity: 1;
                          fill: currentColor;
                          --BaseIcon-color: #333333;
                        "
                        class="BaseIcon IdsButtonFab__contentsIcon"
                        data-v-6d2bd019=""
                        data-v-0b9311f9=""
                      >
                        <g clip-path="url(#clip0_124_2944)">
                          <path
                            d="M3.96869 11.7197L11.4687 4.21967C11.7616 3.92678 12.2365 3.92678 12.5293 4.21967L20.0293 11.7197L18.9687 12.7803L12.749 6.5606V20H11.249V6.56072L5.02935 12.7803L3.96869 11.7197Z"
                          ></path>
                        </g>
                        <defs>
                          <clipPath id="clip0_124_2944">
                            <rect width="24" height="24"></rect>
                          </clipPath>
                        </defs>
                      </svg>
                    </div>
                    <!--]--><!---->
                  </div>
                </div>
                <!--]--></button
              ><!--]-->
            </div>
          </div>
          <!----><!----><!----><!---->
        </div>
      </div>
      <!--]-->
    </div>
  </div>
</template>

<script>
export default {
  name: "CouponComponent",
};
</script>

<style lang="scss" scoped></style>
